...
public class Comment_newsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public Comment_newsServlet() {
		super();
	}
	...
	/**
	 * 请求头：包含携带sessionid的cookie 处理功能：接收客户端的评论信息，存入数据库
	 ...
	 * 处理功能：接收客户端传来的评论id，从数据库中删除指定id的评论 请求参数：id(long)
	 * 返回参数：status(int) 状态码说明： 401---->用户没有权限进行此操作 200---->成功从数据库删除指定id的评论
	 * 500---->删除失败，服务器出错
	 */
	protected void doDelete(HttpServletRequest request, HttpServletResponse response)
		...
		if (check_delete(request, id)) {
			deleteComment_news(request, response, id);
		} else {// 权限不够
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 401);
			String json = jsonObject.toJSONString();
				...
				if (c.getName().contentEquals("JSESSIONID")) {
					...
					if (session != null) {
						...
						if (json == null)
							...
							Comment_news comment = myBatiser.selectCommentById(id);
							if (comment != null && user_id.contentEquals(comment.getUser_id()))
								return true;
						}
					}
				}
	...
	private void getComments(HttpServletRequest request, HttpServletResponse response)
		...
		// 从数据库获取评论
		List<Comment_news> comments = myBatiser.selectCommentByNews_id(news_id);
		...
		for(int i=0;i<comments.size();i++) {
			...
			comments.get(i).setGender((user.getGender()==0)?"未知":(user.getGender()==1?"男":"女"));
		}
	...
	private void postComment(HttpServletRequest request, HttpServletResponse response)
		...
		// 构建Comment对象实例
		Comment_news comment = new Comment_news();
